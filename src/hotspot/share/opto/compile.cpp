...
const TypePtr *Compile::flatten_alias_type( const TypePtr *tj ) const {
  ...
  if( ta && is_known_inst ) {
    ...
    if( offset != Type::OffsetBot ) {
      if( ta->const_oop() ) { // MethodData* or Method*
        ...
        ptr = TypePtr::BotPTR;
#if INCLUDE_SHENANDOAHGC
      } else if (offset == ShenandoahForwarding::byte_offset() && UseShenandoahGC) {
  ...
  if( to && _AliasLevel >= 2 && to != TypeOopPtr::BOTTOM ) {
    ...
    } else if (SHENANDOAHGC_ONLY((offset != ShenandoahForwarding::byte_offset() || !UseShenandoahGC) &&) (offset < 0 || offset >= k->size_helper() * wordSize)) {
  ...
  assert( (offset != Type::OffsetBot && tj->base() != Type::AryPtr) ||
          ...
          (UseShenandoahGC SHENANDOAHGC_ONLY(&& offset == ShenandoahForwarding::byte_offset() && tj->base() == Type::AryPtr)),
